...
# Process names that identify Java / Minecraft instances
JAVA_PROCESS_NAMES = {"java.exe", "javaw.exe"}


class ScannerThread(QThread):
    # Emitted when the scan finishes, carries the list of found processes
    finished_scan = pyqtSignal(list)
    # Emitted to push status updates to the UI during scanning
    ...
    def run(self):
        ...
        try:
            ...
            self.status_update.emit(
                "success", f"Scan complete — {len(processes)} process(es) found"
            )
